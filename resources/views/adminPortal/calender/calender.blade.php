    ...
    <style>
        ...
        * {
        ...
        .popup-overlay {
            ...
            position: fixed;
            inset: 0;
            background: radial-gradient(circle at 20% 20%, rgba(239, 68, 68, 0.08), transparent 45%), rgba(15, 23, 42, 0.65);
            z-index: 1000;
            align-items: center;
            justify-content: center;
            padding: 16px;
        ...
        .view-attachment-link:hover {
            ...
            box-shadow: 0 12px 28px rgba(44, 90, 160, 0.4);
            text-decoration: none;
            color: white;
            background: linear-gradient(135deg, #1e40af 0%, #1e3a8a 100%);
        }

        .view-attachment-link:active {
            transform: translateY(-2px);
        }
        ...
        <div class="appointments-sidebar">
            <div class="sidebar-header">
                ...
                <div class="logo-section">
                    ...
                    <div>
                        <h3 style="margin: 0; color: var(--dark-text);">TGR Africa</h3>
            ...
            <div class="appointments-list">
                <div class="search-container">
                    ...
                    <input type="text" class="search-input" id="event-search" placeholder="Search events by title, location, date, or description...">
                </div>

                <div class="appointments-header">
                    <h2>Upcoming Appointments</h2>
                    <span class="appointment-count" id="visible-count">{{ $events->count() }}</span>
                </div>

                @if($events->count() > 0)
                    @foreach($events as $event)
                        <div class="appointment-item priority-{{ $event->priority ?? 'medium' }}"
                             data-title="{{ strtolower($event->event_title) }}"
                            ...
                            <div class="appointment-actions">
                                <button class="btn btn-primary view-btn" data-id="{{ $event->id }}" title="View">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <button class="btn btn-secondary edit-btn" data-id="{{ $event->id }}" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </button>
                                @if($event->status === 'Pending')
                                    <button class="btn btn-complete complete-btn" data-id="{{ $event->id }}" title="Complete">
                                        <i class="fas fa-check"></i>
                                    </button>
                                @endif

                                <button class="btn btn-remove remove-btn" data-id="{{ $event->id }}" title="Remove">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            calendarGrid.addEventListener("click", function(event) {
                ...
                if (dayElement && dayElement.hasAttribute('data-date')) {
                    ...
                    if (dayElement.classList.contains('past-date')) {
            ...
            searchInput.addEventListener('input', function() {
                ...
                if (visibleItems === 0 && searchTerm !== '') {
                    if (!noResultsMsg) {
                        ...
                        `;
                        appointmentsList.appendChild(noResultsMsg);
                    }
        ...
        function viewEvent(eventId) {
            ...
            .then(data => {
                ...
                const statusColor = event.status === 'Complete' ? '#10b981' : '#f59e0b';
                const eventDateTime = new Date(event.event_date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });

                let html = `
                    <div class="view-section">
                        <h3 class="view-event-title">${event.event_title}</h3>
                        <div class="view-badge-group">
                            <span class="priority-badge priority-${event.priority}">${event.priority.toUpperCase()}</span>
                            <span style="padding: 6px 14px; border-radius: 20px; background: ${statusColor}; color: white; font-size: 11px; font-weight: 700; text-transform: uppercase;">${event.status}</span>
                        </div>
                    </div>
                        ...
                        <div class="view-detail-grid">
                            <div class="view-detail-item">
                                ...
                                <div class="view-detail-content">
                                    ...
                                    <div class="view-detail-value">${eventDateTime}</div>
                                </div>
                            </div>
                            <div class="view-detail-item">
                                <i class="fas fa-clock"></i>
                                <div class="view-detail-content">
                                    ...
                                    <div class="view-detail-value">${event.event_time}</div>
                                </div>
                            </div>
                            ${event.location ? `
                            <div class="view-detail-item">
                                ...
                                <div class="view-detail-content">
                                    ...
                                    <div class="view-detail-value">${event.location}</div>
                                </div>
                            </div>
                            ` : ''}
                            ${event.event_people ? `
                            <div class="view-detail-item">
                                ...
                                <div class="view-detail-content">
                                    <div class="view-detail-label">Attendees</div>
                                    <div class="view-detail-value">${event.event_people}</div>
                                </div>
                            </div>
                            ` : ''}
                        </div>
                    ...
                    <div class="view-section">
                        ...
                        <div class="view-description-box">${event.description}</div>
                    </div>
                    ` : ''}
                    ...
                    <div class="view-section">
                        ...
                        <div style="margin-top: 10px;">
                            <a href="/storage/${event.attachment}" target="_blank" class="view-attachment-link">
                                <i class="fas fa-file-download"></i>
                                <span>Download Attachment</span>
                            </a>
                            <small style="display: block; margin-top: 8px; color: var(--light-text); font-size: 12px;">
                                <i class="fas fa-info-circle"></i> Click to open or download the file
                            </small>
                        </div>
                    </div>
                    ` : ''}
                    ...
                    <div class="view-section">
                        ...
                        <div class="view-emails-box">${event.additional_emails}</div>
                    </div>
                    ` : ''}
                `;
